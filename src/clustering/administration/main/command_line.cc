...
std::string windows_version_string()
{
    ...
    {
    case 0x000A:
        name = "Windows 10, Server 2016";
        break;
    case 0x0306:
        name = "Windows 8.1, Server 2012";
        break;
    case 0x0206:
    ...
    case 0x0006:
        ...
        break;
    case 0x0205:
        name = "Windows XP 64-bit, Server 2003";
        break;
    case 0x0105:
        name = "Windows XP";
        break;
    ...
    local_ip_filter_t filter_type)
{
    ...
    const std::vector<std::string> &bind_options =
        specific_options.size() > 0 ? specific_options : default_options;
...
service_address_ports_t get_service_address_ports(const std::map<std::string, options::values_t> &opts)
{
    ...
    local_ip_filter_t filter =
        exists_option(opts, "--no-default-bind") ? local_ip_filter_t::MATCH_FILTER : local_ip_filter_t::MATCH_FILTER_OR_LOOPBACK;
    ...
    tls_configs_t *tls_configs_out)
{
    ...
    if (!exists_option(opts, "--no-http-admin") &&
        (exists_option(opts, "--http-tls-key") || exists_option(opts, "--http-tls-cert")))
    {
    ...
    if (exists_option(opts, "--driver-tls-key") || exists_option(opts, "--driver-tls-cert") || exists_option(opts, "--driver-tls-ca"))
    {
    ...
    if (exists_option(opts, "--cluster-tls-key") || exists_option(opts, "--cluster-tls-cert") || exists_option(opts, "--cluster-tls-ca"))
    {
    ...
    try
    {
        ...
        else
        {
            ...
            if (!initial_password.empty())
            {
                ...
                if (admin_pair == auth_data.m_users.end() || !static_cast<bool>(admin_pair->second.get_ref()) || admin_pair->second.get_ref()->get_password().is_empty())
                {
                    auto new_admin_pair =
                             ...
                             bool *const result_out)
{
    ...
    // {
    //     PageAllocator::memory_pool->populate_block();
    // }
    if (!new_directory)
    {
                                      ...
                                      int *max_concurrent_io_requests_out)
{
    ...
    if (max_concurrent_io_requests <= 0 || max_concurrent_io_requests > MAXIMUM_MAX_CONCURRENT_IO_REQUESTS)
    {
...
{
    return exists_option(opts, "--direct-io") ? file_direct_io_mode_t::direct_desired : file_direct_io_mode_t::buffered_desired;
}
...
int main_rethinkdb_install_service(int argc, char *argv[])
{
    ...
    try
    {
        ...
        const optional<std::string> runuser_password = get_optional_option(opts, "--runuser-password");
        if (runuser_password)
        {
