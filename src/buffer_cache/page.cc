...
namespace alt
{
    ...
    page_t::page_t(block_id_t _block_id, buf_ptr_t buf,
                   page_cache_t *page_cache, bool RDMA_tmp)
        : block_id_(_block_id),
          loader_(nullptr),
          buf_(std::move(buf)),
                                           ...
                                           buf_ptr_t buf)
    {
        ...
        rassert(block_token.has());
        {
            ...
            page->loader_ = nullptr;
            if (page_cache->getPageMap()->port_number == 6001)
            {
                size_t offset = page_cache->getPageMap()->isBlockIDAvailable(page->block_id_);
                if (offset == static_cast<size_t>(-1))
                {
                    void *tmp_page_buffer = page->get_page_buf(page_cache);
                    if (tmp_page_buffer != nullptr)
                    {
        ...
        cache_account_t *account)
    {
        ...
        scoped_ptr_t<deferred_block_token_t> block_token_ptr = deferred_loader->extract_block_token_ptr();
                                             ...
                                             page_cache_t *page_cache)
    {
        ...
        rassert(evicter->evicted_category() == evicter->correct_eviction_category(page));
#endif
    }
    ...
    {
        eviction_bag_t *old_bag = acq->page_cache()->evicter().correct_eviction_category(this);
    ...
    void *page_t::get_ser_buffer(page_cache_t *page_cache)
    {
        rassert(buf_.has());
